...
from common_segments import common_segment_ibd, common_segment_ibd_by_founders
...
class SharedSegmentDetectorFounder:
    """
    This class compares two genomes and returns the # of basepairs
    they share IBD. It handles detection cutoffs.
    """
    ...
    def shared_segment_length(self, genome_a, genome_b):
        ...
        for founder, segments in segments_by_founder.items():
            ...
            length_above = np.sum(lengths[above_base_cutoff & cm_cutoff])
            if length_above > 0:
                above_cutoffs[founder] = length_above

        return above_cutoffs
